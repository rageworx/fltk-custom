...
class Fl_D2D_Copy_Surface_Driver : public Fl_Copy_Surface_Driver 
{
    friend class Fl_Copy_Surface_Driver;
    protected:
        HDC oldgc;
        HDC gc;
        Fl_D2D_Copy_Surface_Driver(int w, int h);
        ~Fl_D2D_Copy_Surface_Driver();
        void set_current();
...
{
    return new Fl_D2D_Copy_Surface_Driver(w, h);
}


Fl_D2D_Copy_Surface_Driver::Fl_D2D_Copy_Surface_Driver(int w, int h) : Fl_Copy_Surface_Driver(w, h) 
{
    ...
    ((Fl_D2D_Graphics_Driver*)driver())->scale(scaling);
...
Fl_D2D_Copy_Surface_Driver::~Fl_D2D_Copy_Surface_Driver() 
{
    if (oldgc == (HDC)Fl_Surface_Device::surface()->driver()->gc()) 
        oldgc = NULL;
    ...
    if ( hmf != NULL ) 
    {
        if ( OpenClipboard (NULL) )
        {
            ...
            int W = Fl_Scalable_Graphics_Driver::floor(width, scaling);
            int H = Fl_Scalable_Graphics_Driver::floor(height, scaling);
            RECT rect = {0, 0, W, H};
...
void Fl_D2D_Copy_Surface_Driver::set_current() 
{
    driver()->gc(gc);
    fl_window = (Window)1;
...
void Fl_D2D_Copy_Surface_Driver::translate(int x, int y) 
{
    ((Fl_D2D_Graphics_Driver*)driver())->translate_all(x, y);
}

void Fl_D2D_Copy_Surface_Driver::untranslate() 
{
