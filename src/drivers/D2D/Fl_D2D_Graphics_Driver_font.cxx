...
const char* Fl_D2D_Graphics_Driver::get_font_name(Fl_Font fnum, int* ap) 
{
    ...
    if (!f->fontname[0]) 
    {
        ...
        if (type & FL_BOLD) 
            strlcat(f->fontname, " bold", ENDOFBUFFER);
        if (type & FL_ITALIC) 
            strlcat(f->fontname, " italic", ENDOFBUFFER);
...
enumcbw( CONST LOGFONTW    *lpelf,
         ...
         LPARAM           p) 
{
    if (!p && lpelf->lfCharSet != ANSI_CHARSET) 
        return 1;
    
    char *n = NULL;
    size_t l = wcslen(lpelf->lfFaceName);
    // measure the string
    unsigned dstlen = fl_utf8fromwc(n, 0, (wchar_t*)lpelf->lfFaceName, (unsigned) l) + 1; 
    n = (char*) malloc(dstlen);
    //n[fl_unicode2utf((wchar_t*)lpelf->lfFaceName, l, n)] = 0;
    // convert the string
    dstlen = fl_utf8fromwc(n, dstlen, (wchar_t*)lpelf->lfFaceName, (unsigned) l); 
    n[dstlen] = 0;

    // skip if one of our built-in fonts
    for (int i=0; i<FL_FREE_FONT; i++) 
        if (!strcmp(Fl::get_font_name((Fl_Font)i),n)) 
        {
            free(n);
            return 1;
        }
        
    char buffer[LF_FACESIZE + 1] = {0};
    strcpy(buffer+1, n);
    
    buffer[0] = ' '; 
    Fl::set_font((Fl_Font)(fl_free_font++), fl_strdup(buffer));
...
Fl_Font Fl_D2D_Graphics_Driver::set_fonts(const char* xstarname) 
{
...
int Fl_D2D_Graphics_Driver::get_font_sizes(Fl_Font fnum, int*& sizep) {
  nbSize = 0;
...
const char *Fl_D2D_Graphics_Driver::font_name(int num) {
  return fl_fonts[num].name;
}

void Fl_D2D_Graphics_Driver::font_name(int num, const char *name) {
...
#ifndef FL_DOXYGEN
Fl_D2D_Font_Descriptor::Fl_D2D_Font_Descriptor(const char* name, Fl_Fontsize fsize) : Fl_Font_Descriptor(name,fsize) {
...
Fl_D2D_Font_Descriptor::~Fl_D2D_Font_Descriptor() {
#if HAVE_GL
...
static Fl_D2D_Font_Descriptor* find(Fl_Font fnum, Fl_Fontsize size, int angle) {
  Fl_Fontdesc* s = fl_fonts+fnum;
  if (!s->name) s = fl_fonts; // use 0 if fnum undefined
  Fl_D2D_Font_Descriptor* f;
  for (f = (Fl_D2D_Font_Descriptor*)s->first; f; f = (Fl_D2D_Font_Descriptor*)f->next)
    if (f->size == size && f->angle == angle) return f;
  f = new Fl_D2D_Font_Descriptor(s->name, size);
  f->next = s->first;
  s->first = f;
  return f;
}
...
static void fl_font(Fl_Graphics_Driver *driver, Fl_Font fnum, Fl_Fontsize size, int angle) {
  ...
  if (fnum == driver->Fl_Graphics_Driver::font() && size == ((Fl_D2D_Graphics_Driver*)driver)->size_unscaled() && angle == fl_angle_) return;
  fl_angle_ = angle;
  driver->Fl_Graphics_Driver::font(fnum, size);
  driver->font_descriptor( find(fnum, size, angle) );
}

void Fl_D2D_Graphics_Driver::font_unscaled(Fl_Font fnum, Fl_Fontsize size) {
  fl_font(this, fnum, size, 0);
}

int Fl_D2D_Graphics_Driver::height_unscaled() {
  Fl_D2D_Font_Descriptor *fl_fontsize = (Fl_D2D_Font_Descriptor*)font_descriptor();
  if (fl_fontsize) return (fl_fontsize->metr.tmAscent + fl_fontsize->metr.tmDescent);
  else return -1;
}

int Fl_D2D_Graphics_Driver::descent_unscaled() {
  Fl_D2D_Font_Descriptor *fl_fontsize = (Fl_D2D_Font_Descriptor*)font_descriptor();
  if (fl_fontsize) return fl_fontsize->metr.tmDescent;
  else return -1;
}

Fl_Fontsize Fl_D2D_Graphics_Driver::size_unscaled() {
...
double Fl_D2D_Graphics_Driver::width_unscaled(const char* c, int n) {
  int i = 0;
...
double Fl_D2D_Graphics_Driver::width_unscaled(unsigned int c) {
  Fl_D2D_Font_Descriptor *fl_fontsize = (Fl_D2D_Font_Descriptor*)font_descriptor();
  unsigned int r;
  SIZE s;
...
void Fl_D2D_Graphics_Driver::text_extents_unscaled(const char *c, int n, int &dx, int &dy, int &w, int &h) {
...
void Fl_D2D_Graphics_Driver::draw_unscaled(const char* str, int n, int x, int y) {
  ...
  SelectObject(gc_, ((Fl_D2D_Font_Descriptor*)font_descriptor())->fid);
...
void Fl_D2D_Graphics_Driver::draw_unscaled(int angle, const char* str, int n, int x, int y) {
...
void Fl_D2D_Graphics_Driver::rtl_draw_unscaled(const char* c, int n, int x, int y) {
  int wn;
