...
{
    return new Fl_D2D_Graphics_Driver();
}
...
void Fl_D2D_Graphics_Driver::global_gc()
{
    fl_gc = (HDC)gc();
}
 ...
 */
char Fl_D2D_Graphics_Driver::can_do_alpha_blending() 
{
...
void Fl_D2D_Graphics_Driver::copy_offscreen(int x, int y, int w, int h, Fl_Offscreen bitmap, int srcx, int srcy) 
{
    ...
    if (srcx < 0) 
    {
        w += srcx; x -= srcx; srcx = 0;
    }
    
    if (srcy < 0) 
    {
        h += srcy; y -= srcy; srcy = 0;
    }
...
void Fl_D2D_Printer_Graphics_Driver::copy_offscreen(int x, int y, int w, int h, Fl_Offscreen bitmap, int srcx, int srcy) 
{
    Fl_Graphics_Driver::copy_offscreen(x, y, w, h, bitmap, srcx, srcy);
}

BOOL Fl_D2D_Graphics_Driver::alpha_blend_(int x, int y, int w, int h, HDC src_gc, int srcx, int srcy, int srcw, int srch) 
{
    return fl_alpha_blend(gc_, x, y, w, h, src_gc, srcx, srcy, srcw, srch, blendfunc);
}

#if ! defined(FL_DOXYGEN)
void Fl_D2D_Graphics_Driver::copy_offscreen_with_alpha(int x,int y,int w,int h,HBITMAP bitmap,int srcx,int srcy) 
{
    HDC new_gc = CreateCompatibleDC(gc_);
    int save = SaveDC(new_gc);
    SelectObject(new_gc, bitmap);
...
void Fl_D2D_Graphics_Driver::translate_all(int x, int y) 
{
...
void Fl_D2D_Graphics_Driver::untranslate_all() 
{
    if (depth > 0) depth--;
...
void Fl_D2D_Graphics_Driver::add_rectangle_to_region(Fl_Region r, int X, int Y, int W, int H) 
{
...
void Fl_D2D_Graphics_Driver::transformed_vertex0(float x, float y) 
{
...
void Fl_D2D_Graphics_Driver::fixloop() 
{
    // remove equal points from closed path
    while (n>2 && p[n-1].x == p[0].x && p[n-1].y == p[0].y) n--;
}

Fl_Region Fl_D2D_Graphics_Driver::XRectangleRegion(int x, int y, int w, int h) 
{
...
void Fl_D2D_Graphics_Driver::XDestroyRegion(Fl_Region r) 
{
    DeleteObject(r);
}
...
void Fl_D2D_Graphics_Driver::set_spot(int font, int size, int X, int Y, int W, int H, Fl_Window *win)
{
    if (!win) 
        return;
...
void Fl_D2D_Graphics_Driver::scale(float f) 
{
    if (f != scale()) 
    {
        size_ = 0;
 ...
 */
HRGN Fl_D2D_Graphics_Driver::scale_region(HRGN r, float f, Fl_D2D_Graphics_Driver *dr) 
{
    if (r && f != 1) 
    {
...
Fl_Region Fl_D2D_Graphics_Driver::scale_clip(float f) 
{
...
void Fl_D2D_Graphics_Driver::set_current_() 
{
    restore_clip();
}

void Fl_D2D_Graphics_Driver::cache_size(Fl_Image *img, int &width, int &height)
{
    float s = scale();
