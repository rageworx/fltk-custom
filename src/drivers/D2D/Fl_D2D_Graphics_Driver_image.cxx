...
void Fl_D2D_Graphics_Driver::draw_image_unscaled(const uchar* buf, int x, int y, int w, int h, int d, int l)
{
...
void Fl_D2D_Graphics_Driver::draw_image_unscaled(Fl_Draw_Image_Cb cb, void* data,
...
void Fl_D2D_Graphics_Driver::draw_image_mono_unscaled(const uchar* buf, int x, int y, int w, int h, int d, int l)
{
    if (fl_abs(d)&FL_IMAGE_WITH_ALPHA) 
    {
        d ^= FL_IMAGE_WITH_ALPHA;
...
void Fl_D2D_Graphics_Driver::draw_image_mono_unscaled(Fl_Draw_Image_Cb cb, void* data,
                   int x, int y, int w, int h,int d) 
{
    if (fl_abs(d)&FL_IMAGE_WITH_ALPHA) 
    {
...
Fl_Bitmask Fl_D2D_Graphics_Driver::create_bitmask(int w, int h, const uchar *data) 
{
...
void Fl_D2D_Graphics_Driver::delete_bitmask(Fl_Bitmask bm) 
{
    DeleteObject((HGDIOBJ)bm);
}

void Fl_D2D_Graphics_Driver::draw_fixed(Fl_Bitmap *bm, int X, int Y, int W, int H, int cx, int cy) 
{
    X = this->floor(X);
    Y = this->floor(Y);
    cache_size(bm, W, H);
...
Fl_D2D_Printer_Graphics_Driver::transparent_f_type Fl_D2D_Printer_Graphics_Driver::TransparentBlt() 
{
    HMODULE hMod;
...
void Fl_D2D_Printer_Graphics_Driver::draw_bitmap(Fl_Bitmap *bm, int XP, int YP, int WP, int HP, int cx, int cy) 
{
    int X, Y, W, H;
    ...
    HDC tempdc;
    int save;
    // algorithm for bitmap output to Fl_D2D_Printer
...
void Fl_D2D_Graphics_Driver::cache(Fl_RGB_Image *img)
{
...
void Fl_D2D_Graphics_Driver::draw_fixed(Fl_RGB_Image *img, int X, int Y, int W, int H, int cx, int cy) {
  X = this->floor(X);
  Y = this->floor(Y);
  cache_size(img, W, H);
  cx = this->floor(cx); cy = this->floor(cy);
...
void Fl_D2D_Graphics_Driver::draw_rgb(Fl_RGB_Image *rgb, int XP, int YP, int WP, int HP, int cx, int cy) {
...
void Fl_D2D_Printer_Graphics_Driver::draw_rgb(Fl_RGB_Image *rgb, int XP, int YP, int WP, int HP, int cx, int cy) {
  if (Fl_Graphics_Driver::start_image(rgb, XP, YP, WP, HP, cx, cy, XP, YP, WP, HP)) {
    return;
  }
  XFORM old_tr, tr;
...
void Fl_D2D_Graphics_Driver::uncache(Fl_RGB_Image*, fl_uintptr_t &id_, fl_uintptr_t &mask_)
{
  if (id_) {
...
void Fl_D2D_Graphics_Driver::cache(Fl_Bitmap *bm) {
  int *pw, *ph;
  cache_w_h(bm, pw, ph);
  *pw = bm->data_w();
  *ph = bm->data_h();
...
void Fl_D2D_Graphics_Driver::draw_fixed(Fl_Pixmap *pxm, int X, int Y, int W, int H, int cx, int cy) {
  X = this->floor(X);
  Y = this->floor(Y);
  cache_size(pxm, W, H);
  cx = this->floor(cx); cy = this->floor(cy);
...
/*  ===== Implementation note about how Fl_Pixmap objects get printed under Windows =====
 Fl_Pixmap objects are printed with the print-specific Fl_D2D_Printer_Graphics_Driver
 which uses the TransparentBlt() system function that can scale the image and treat one
 of its colors as transparent.
 Fl_D2D_Printer_Graphics_Driver::draw_pixmap(Fl_Pixmap *,...) sets need_pixmap_bg_color,
 ...
 reset to 0.
 Fl_D2D_Graphics_Driver::make_unused_color_() which does the color computation mentionned
 above is implemented in file src/fl_draw_pixmap.cxx
 */
void Fl_D2D_Printer_Graphics_Driver::draw_pixmap(Fl_Pixmap *pxm, int XP, int YP, int WP, int HP, int cx, int cy) {
  int X, Y, W, H;
...
void Fl_D2D_Graphics_Driver::make_unused_color_(uchar &r, uchar &g, uchar &b, int color_count, void **data) {
...
void Fl_D2D_Graphics_Driver::cache(Fl_Pixmap *img) 
{
...
void Fl_D2D_Graphics_Driver::uncache_pixmap(fl_uintptr_t offscreen) 
{
