...
/**
  \file Fl_D2D_Graphics_Driver_arci.cxx
...
void Fl_D2D_Graphics_Driver::arc_unscaled(int x, int y, int w, int h, double a1, double a2) 
{
    if (w <= 0 || h <= 0) 
        return;
    ...
    if (fabs(a1 - a2) < 90) 
    {
        ...
        Arc(gc_, int(x), int(y), int(x+w), int(y+h), xa, ya, xb, yb);
}

void Fl_D2D_Graphics_Driver::pie_unscaled(int x, int y, int w, int h, double a1, double a2) 
{
    if (w <= 0 || h <= 0) 
        return;
    ...
    if (fabs(a1 - a2) < 90) 
    {
        if (xa == xb && ya == yb) 
        {
            ...
            Pie(gc_, int(x), int(y), int(x+w), int(y+h), xa, ya, xb, yb);
    } 
    else 
        Pie(gc_, int(x), int(y), int(x+w), int(y+h), xa, ya, xb, yb);
}
