...
{
    if (savepen) 
        SelectObject((HDC)fl_graphics_driver->gc(), savepen);
    DeleteObject(tmppen);
...
static void clear_xmap(Fl_XMap& xmap) 
{
    if (xmap.pen) 
    {
        ...
        HGDIOBJ oldpen = SelectObject(gc, tmppen);
        // Put it back if it is not the one we are about to delete
        if(oldpen != xmap.pen) SelectObject(gc, oldpen);
...
static void set_xmap(Fl_XMap& xmap, COLORREF c, int lw) 
{
    ...
    if (xmap.pen) 
    {
        ...
        HGDIOBJ oldpen = SelectObject(gc,GetStockObject(BLACK_PEN));
...
void Fl_D2D_Graphics_Driver::color(Fl_Color i) 
{
    if (i & 0xffffff00) 
    {
...
void Fl_D2D_Graphics_Driver::color(uchar r, uchar g, uchar b) 
{
    static Fl_XMap xmap;
...
#define FL_N_BRUSH 16
    ...
    if (i != -1) 
    { 
        // if the brush was allready allocated
        if (brushes[i].brush == NULL) 
            goto CREATE_BRUSH;
        
        if ( (++brushes[i].usage) > 32000 ) 
        { 
            // keep a usage statistic
    ...
    else 
    {
        ...
        i = imin;
        // get a stock brush
        HGDIOBJ tmpbrush = GetStockObject(BLACK_BRUSH);
        // load in into current context
        HGDIOBJ oldbrush = SelectObject(gc,tmpbrush);
        
        if (oldbrush != brushes[i].brush) 
            SelectObject(gc,oldbrush);  /// reload old one
...
void Fl_D2D_Graphics_Driver::free_color(Fl_Color i, int overlay) 
{
    if (overlay) return; // do something about GL overlay?
    clear_xmap(fl_xmap[i]);
}

void Fl_D2D_Graphics_Driver::set_color(Fl_Color i, unsigned c) 
{
    if (fl_cmap[i] != c) 
    {
        clear_xmap(fl_xmap[i]);
